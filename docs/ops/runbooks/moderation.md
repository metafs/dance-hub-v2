...
`/listing-requests` は匿名の受付窓口である。Turnstile を通過した要請は
`/admin/withdrawals` の「受付待ちの要請」に入る。連絡先と要請内容は内部情報であり、公開
しない。削除要請は該当 Event ID を同画面の取り下げフォームに転記して処理し、受付カードを
完了にする。理由は内部記録であり、公開されない。
...
`listing_origin = proxy` の Event は公開情報から入力した在庫である。画像は DB trigger で
追加できない。公開ページの代理入力表示から修正・削除の受付へ案内されるため、公開後すみやかに
主催者へ同じ窓口を手動で通知する（C-6）。受付後は通常の Revision 審査または取り下げを優先して
処理する。
