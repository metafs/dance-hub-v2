...
**`p8ce.dance` の取得可否を先に確認する。** 取得できない場合は名称ごと再検討する（ADR-0021 Revisit when）。以降のすべての作業はこの確認の後に行う。
...
- [ ] ロゴ正本 SVG をリポジトリに追加する（`docs/brand/identity.md` Open items）
...
1. 前提条件を満たす。`p8ce.dance` が取れなければ中止する。
