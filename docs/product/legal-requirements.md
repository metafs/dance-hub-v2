...
3 つの文書は公開ページとして用意してあり、公開ページのフッターから常にリンクしている（2026-09-26）。

| 文書 | URL | 構成 |
| --- | --- | --- |
| 利用規約 | `/terms` | `src/features/legal/documents.ts` の `termsOfService` |
| プライバシーポリシー | `/privacy` | 同 `privacyPolicy` |
| 運営者情報 | `/operator` | 同 `operatorInformation` |

- 節の見出しは下の表の項目から組んだ。条文はまだ無く、各節は「準備中」と表示する。ほかの文書で決まっている内容（審査の基準、削除・修正の依頼先、G-5）は、その文書へリンクする。
- 各文書は `draft: true` の間、「準備中」の告知を出し、`noindex` で検索エンジンに登録させない。条文を確定したら、その文書の節に本文を入れ、`draft` を `false` にする。一般公開（ロードマップ M8）は 3 つとも `draft: false` になってから行う。
