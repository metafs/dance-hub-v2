...
import { restoreEvent, withdrawEvent } from "@/features/moderation/commands";
import { requirePlatformAdmin } from "@/features/moderation/policy";
import {
  getOpenListingRequests,
  getWithdrawnEvents,
...
export default async function WithdrawalQueue({
  ...
  const { data: withdrawn, error } = await getWithdrawnEvents(supabase);
  if (error) throw new Error("Withdrawn events could not be loaded.");
  const { data: requests, error: requestsError } = await getOpenListingRequests(supabase);
