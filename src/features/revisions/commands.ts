...
export async function createEventDraftWithState(
  ...
  if (!parsed.success) return { status: "error", message: "入力内容を確認してください。", fieldErrors: parsed.errors };
  ...
  if (eventError || !event) return { status: "error", message: "下書きを作成できませんでした。", fieldErrors: { form: ["時間をおいてもう一度お試しください。"] } };
  ...
  if (revisionError || !revision) return { status: "error", message: "下書きを作成できませんでした。", fieldErrors: { form: ["入力内容を確認して、もう一度お試しください。"] } };
  const contentError = await replaceEventRevisionContent(supabase, revision.id, revisionContent);
  if (contentError) return { status: "error", message: "関連情報を保存できませんでした。", fieldErrors: { form: ["Artist、会場、料金、リンク、画像の内容を確認してください。"] } };
  revalidatePath(eventPath(organizationId));
  redirect(`${eventPath(organizationId, event.id)}?created=1`);
}
...
export async function mutateEventDraftWithState(
  ...
  if (intent !== "save" && intent !== "submit") return { status: "error", message: "操作を確認できませんでした。", fieldErrors: { form: ["ページを再読み込みしてください."] } };
  const submit = intent === "submit";
  const parsed = parseEventRevisionInput(formData, { forSubmission: submit });
  if (!parsed.success) return { status: "error", message: submit ? "審査提出に必要な項目を確認してください。" : "入力内容を確認してください。", fieldErrors: parsed.errors };

  const { fields, content: revisionContent } = parsed.data;
  const { error: revisionError } = await supabase.from("event_revisions").update(fields).eq("id", revisionId).eq("event_id", eventId);
  if (revisionError) return { status: "error", message: "このRevisionを保存できませんでした。編集可能な状態か確認してください。", fieldErrors: { form: ["保存対象が見つからないか、編集できない状態です。"] } };
  const contentError = await replaceEventRevisionContent(supabase, revisionId, revisionContent);
  if (contentError) return { status: "error", message: "関連情報を保存できませんでした。", fieldErrors: { form: ["Artist、会場、料金、リンク、画像の内容を確認してください。"] } };
  if (submit) {
