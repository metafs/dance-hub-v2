...
export default async function EventReviewQueue({
  ...
  const [{ data: revisions, error: revisionError }, { data: cancellations, error: cancellationError }] = await Promise.all([
    supabase
      ...
      .select("id, event_id, title, description, event_type, application_deadline, created_at, events!event_revisions_event_id_fkey!inner(owner_organization_id, organizations(name))")
      .eq("status", "in_review")
      .order("created_at"),
    supabase
      .from("event_cancellation_requests")
      .select("id, event_id, requested_reason, created_at, events!event_cancellation_requests_event_id_fkey!inner(owner_organization_id, organizations(name))")
      .eq("status", "in_review")
      .order("created_at"),
  ]);
