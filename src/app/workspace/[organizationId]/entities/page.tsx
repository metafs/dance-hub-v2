...
import { createArtistCandidate, createVenueCandidate, requestArtistChange } from "./actions";
...
export default async function EntityWorkspacePage({ params, searchParams }: { params: Promise<{ organizationId: string }>; searchParams: Promise<{ created?: string; updated?: string; error?: string }> }) {
  ...
  const [{ data: artists }, { data: venues }, { data: artistCandidates }, { data: venueCandidates }] = await Promise.all([
    supabase.from("artists").select("id, name, artist_type, website_url").order("name").limit(30),
    supabase.from("venues").select("id, name, prefecture, address_line1").order("name").limit(30),
    supabase.from("artist_candidates").select("id, name, artist_type, status, decision_reason").eq("creator_organization_id", organizationId).order("created_at", { ascending: false }),
    supabase.from("venue_candidates").select("id, name, prefecture, status, decision_reason").eq("creator_organization_id", organizationId).order("created_at", { ascending: false }),
  ]);
