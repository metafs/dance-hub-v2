...
export function EventFields({ artists, venues, festivalParents, ticketOffers = [], canUploadMainImage = true, hasMainImage = false, defaults, errors }: Props) {
  ...
  return <>
    ...
    <fieldset><legend>問い合わせ先（審査提出時は必須）</legend><div className="field"><label>手段<select aria-describedby={describedBy("contactKind")} aria-invalid={invalid("contactKind")} name="contactKind" defaultValue={value("contactKind")}><option value="">選択してください</option><option value="website">公式サイト</option><option value="social">SNS</option><option value="email">メールアドレス</option></select></label><FieldError errors={errors} name="contactKind"/></div><div className="field"><label>問い合わせ先<input aria-describedby={describedBy("contactValue")} aria-invalid={invalid("contactValue")} name="contactValue" defaultValue={value("contactValue")} maxLength={500} placeholder="https://… または contact@example.com"/></label><FieldError errors={errors} name="contactValue"/></div></fieldset>
    ...
    <fieldset><legend>メイン画像（任意）</legend>{canUploadMainImage ? <><div className="field"><label>画像ファイル<input accept="image/jpeg,image/png,image/webp" aria-describedby={describedBy("image")} aria-invalid={invalid("image")} name="image" type="file"/></label><FieldError errors={errors} name="image"/></div><p className="field-help">{hasMainImage ? "現在この Revision には画像が設定されています。新しく選ぶと差し替わります。" : "この Revision にはまだ画像がありません。"}登録する場合は代替テキストが必要です。JPEG、PNG、WebP。10MBまで。</p><div className="field"><label>代替テキスト<input aria-describedby={describedBy("imageAlt")} aria-invalid={invalid("imageAlt")} name="imageAlt" defaultValue={value("imageAlt")} maxLength={500}/></label><FieldError errors={errors} name="imageAlt"/></div></> : <p className="field-help">画像と代替テキストは、下書きを作成したあとEventの編集画面から設定します。</p>}</fieldset>
