...
export function EventRevisionForm(props: Props) {
  ...
  return <form action={action} className="form-card form-stack" noValidate>
    ...
    <EventFields key={state.values ? JSON.stringify(state.values) : "initial"} artists={props.artists} venues={props.venues} festivalParents={props.festivalParents} ticketOffers={state.values?.ticketOffers ?? props.ticketOffers} imageEditable hasMainImage={props.hasMainImage} defaults={defaults} errors={state.fieldErrors}/>
